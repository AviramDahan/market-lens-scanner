Daily Performance Summary

Date: 2026-09-10
Total scans: 1
Tickers scanned: 134
BUY_SIMULATED: 0
WATCH_READY: 0
WATCH_READY unique tickers: 0
WATCH_REVIEW: 27
WATCH_REVIEW unique tickers: 27
WATCH_READY session split: regular=0, off_hours=0, unknown=0
WATCH_READY conversion: 0 converted / 0 staged
WATCH: 27
SKIP: 101
NO_TRADE: 72
Realized PnL: 0.0
Unrealized PnL: 939.24
Portfolio value: 101586.53
Daily recorded-equity change (%): -0.0053
Daily return status: RECORDED_EQUITY_CHANGE
Daily reference date: 2026-09-09
Cumulative recorded-equity change (%): 1.5865
Trade metric source: COMPLETED_TRADE_LIFECYCLE
Best ticker: APP
Worst ticker: AEP
Best shadow strategy: INSUFFICIENT_OUTCOMES
Worst shadow strategy: INSUFFICIENT_OUTCOMES
Shadow would-buy counts:

Top rejected candidates:
- ASML: SKIP score=0.62 reason=SKIP: Position size blocked by cash, exposure, or risk limits. Market regime NEUTRAL; sector NEUTRAL; net R/R 2.72.
- KMI: WATCH score=0.62 reason=WATCH: Technical setup detected, but weighted risk/reward 1.47 is below minimum 2.00. Market regime NEUTRAL; sector STRONG; net R/R 1.00.
- GS: SKIP score=0.56 reason=SKIP: Position size blocked by cash, exposure, or risk limits. Market regime NEUTRAL; sector NEUTRAL; net R/R 2.15.
- MA: SKIP score=0.52 reason=SKIP: Position size blocked by cash, exposure, or risk limits. Market regime NEUTRAL; sector NEUTRAL; net R/R 2.09.
- EQIX: SKIP score=0.52 reason=SKIP: Utilities / Real Assets sector regime is weak (24/100); skip new entry. Market regime NEUTRAL; sector WEAK; net R/R 1.54.
- NVDA: WATCH score=0.51 reason=WATCH: Technical setup detected, but weighted risk/reward 1.81 is below minimum 2.00. Market regime NEUTRAL; sector STRONG; net R/R 1.27.
- V: SKIP score=0.5 reason=SKIP: Position size blocked by cash, exposure, or risk limits. Market regime NEUTRAL; sector NEUTRAL; net R/R 1.56.
- BRK-B: WATCH score=0.49 reason=WATCH: Technical setup detected, but weighted risk/reward 1.71 is below minimum 2.00. Market regime NEUTRAL; sector NEUTRAL; net R/R 1.13.
- JNJ: SKIP score=0.48 reason=SKIP: Position size blocked by cash, exposure, or risk limits. Market regime NEUTRAL; sector NEUTRAL; net R/R 1.81.
- KO: SKIP score=0.48 reason=SKIP: Consumer sector regime is weak (21/100); skip new entry. Market regime NEUTRAL; sector WEAK; net R/R 1.35.

Recommendations:
- Keep collecting shadow data; no strategy changes are recommended from this sample alone.