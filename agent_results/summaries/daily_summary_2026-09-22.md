Daily Performance Summary

Date: 2026-09-22
Total scans: 28
Tickers scanned: 3825
BUY_SIMULATED: 1
WATCH_READY: 0
WATCH_READY unique tickers: 0
WATCH_REVIEW: 727
WATCH_REVIEW unique tickers: 55
WATCH_READY session split: regular=0, off_hours=0, unknown=0
WATCH_READY conversion: 0 converted / 0 staged
WATCH: 727
SKIP: 2992
NO_TRADE: 2278
Realized PnL: -56.39
Unrealized PnL: 939.84
Portfolio value: 101531.65
Daily recorded-equity change (%): -0.2095
Daily return status: RECORDED_EQUITY_CHANGE
Daily reference date: 2026-09-21
Cumulative recorded-equity change (%): 1.5316
Trade metric source: COMPLETED_TRADE_LIFECYCLE
Best ticker: AVGO
Worst ticker: ABBV
Best shadow strategy: INSUFFICIENT_OUTCOMES
Worst shadow strategy: INSUFFICIENT_OUTCOMES
Shadow would-buy counts:
- BREAKOUT_CONTINUATION: 2
- FIB_STOP_075_ATR: 18
- FIB_STOP_100_ATR: 3
- FIB_STRUCTURE_STOP: 10
- TREND_PULLBACK_RECLAIM: 17

Top rejected candidates:
- KMI: WATCH score=0.5574 reason=WATCH: Technical setup detected, but weighted risk/reward 1.45 is below minimum 2.00. Market regime NEUTRAL; sector STRONG; net R/R 0.99.
- QCOM: WATCH score=0.5468 reason=WATCH: Technical setup detected, but weighted risk/reward 1.16 is below minimum 2.00. Market regime NEUTRAL; sector STRONG; net R/R 0.90.
- MSFT: WATCH score=0.5299 reason=WATCH: Technical setup detected, but weighted risk/reward 1.72 is below minimum 2.00. Market regime NEUTRAL; sector STRONG; net R/R 1.36.
- CAT: SKIP score=0.5187 reason=SKIP: Industrials sector regime is weak (16/100); skip new entry. Market regime NEUTRAL; sector WEAK; net R/R 5.35.
- QBTS: WATCH score=0.5179 reason=WATCH: Neutral market requires stronger normalized quality (43.12/100).
- ORCL: WATCH score=0.5063 reason=WATCH: NEUTRAL market requires setup score (0.51 < 0.55).
- WMB: WATCH score=0.5019 reason=WATCH: Technical setup detected, but weighted risk/reward 1.89 is below minimum 2.00. Market regime NEUTRAL; sector STRONG; net R/R 1.54.
- EQIX: SKIP score=0.4976 reason=SKIP: Utilities / Real Assets sector regime is weak (11/100); skip new entry. Market regime NEUTRAL; sector WEAK; net R/R 1.09.
- DHR: WATCH score=0.4965 reason=WATCH: Technical setup detected, but weighted risk/reward 1.45 is below minimum 2.00. Market regime NEUTRAL; sector STRONG; net R/R 1.05.
- LRCX: WATCH score=0.4952 reason=WATCH: NEUTRAL market requires setup score (0.50 < 0.55).

Recommendations:
- Review shadow would-buy candidates that active gates skipped before changing thresholds.