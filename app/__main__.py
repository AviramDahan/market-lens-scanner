...
def main() -> None:
    parser = argparse.ArgumentParser(prog="app", description="Market Lens swing trade scanner")
    ...
    scan_cmd.add_argument(
        "--chart-dir",
        default="charts",
        help="Directory for chart images (default: charts)",
    )
...
def _write_charts(details: list[ScanDetail], chart_dir: str) -> dict[str, str]:
    from app.charts import write_scan_chart

    output_dir = Path(chart_dir)
    chart_paths: dict[str, str] = {}
    for detail in details:
        path = write_scan_chart(detail, output_dir)
        chart_paths[detail.result.ticker] = str(path)
    return chart_paths


def _print_chart_paths(chart_paths: dict[str, str]) -> None:
    if not chart_paths:
        return
    print("Charts:")
...
def _print_verbose(details: list[ScanDetail], errors: dict[str, str]) -> None:
    ...
    for d in sorted_details:
        ...
        vp_scenario_tags = {
            "price_up_vol_up": "price up / volume up  +8 (bullish confirmation)",
            "price_up_vol_down": "price up / volume down  0  (weak upside)",
            "price_down_vol_down": "price down / volume down  +8 (fading decline - bottom approaching)",
            "price_down_vol_up": "price down / volume up  -10 (strong decline - avoid)",
        }
        vp_tag = vp_scenario_tags.get(d.vp_scenario, d.vp_scenario)
        rs = d.relative_strength
        if rs > 1.3:
            rs_tag = f"{rs:.2f}x  outperforming (+10)"
        elif rs < 0.7:
            rs_tag = f"{rs:.2f}x  underperforming (-8)"
        else:
            rs_tag = f"{rs:.2f}x  in-line (0)"
        ...
        if vsl and vsl.is_valid:
            near_tag = "  OK price near" if vsl.price_near else ""
            sweep_tag = "  OK sweep detected" if vsl.sweep_detected else ""
            fail_tag = "  FAIL accepted below (-20)" if vsl.accepted_below else ""
        ...
        if br is not None:
            holding_tag = "OK price holding above retest level" if br.is_holding else "FAIL price slipping below retest level"
            print(f"  Breakout + Retest")
            print(f"    Resistance : {br.resistance_level:.2f}  ({br.resistance_date})")
            print(f"    Breakout   : {br.breakout_date}  (vol {br.breakout_volume_ratio:.1f}x avg)")
